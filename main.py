...
with st.sidebar:
    page = option_menu(
        menu_title="Navigation", options=["EKG-App", "Power / HR Analyse"], icons=["heart", "activity"], default_index=0
    )
...
if page == "EKG-App":
    ...
    with col1:
        current_user = st.selectbox("Versuchsperson", options=user_list, key="sbVersuchsperson")

    with col2:
        picture_path = get_picture_path(person_data, current_user)
    ...
    if selected_person is not None:
        ...
        selected_test_name = st.selectbox("EKG-Test auswählen", options=list(test_options.keys()))
        ...
        if hr is not None:
            st.metric("Durchschnittliche Herzfrequenz", f"{hr:.1f} bpm")
