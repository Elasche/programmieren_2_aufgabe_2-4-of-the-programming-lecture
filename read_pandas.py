...
def make_power_hr_plot(df, max_hr):
    ...
    df,
    x="time_secounds",
    y=["PowerOriginal", "HeartRate"],
    color_discrete_sequence=["#C084FC", "#EF4444"]
    ) 
     # Zone 1
    fig.add_hrect(
        y0=0,
...
def zone_bar_plot(df, max_hr):
    ...
    z2 = ((df["HeartRate"] >= zone_1) &
        (df["HeartRate"] < zone_2)).sum()
    ...
    z3 = ((df["HeartRate"] >= zone_2) &
        (df["HeartRate"] < zone_3)).sum()
    ...
    z4 = ((df["HeartRate"] >= zone_3) &
        (df["HeartRate"] < zone_4)).sum()
    ...
    zone_df["Text"] = (
        "<b>"
        + zone_df["Leistung"].round(0).astype(int).astype(str)
        + " W</b>"
    )
